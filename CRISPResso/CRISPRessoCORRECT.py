...
def classify_read (row,substitution_positions,insertion_positions_flat,deletion_positions_flat):
    ...
    if include_idxs.intersection(insertion_positions_flat) or  include_idxs.intersection(deletion_positions_flat):
        NHEJ = True
	# Now correct any snps (unless adjacent to indel)
	list_align_seq = list(row.align_seq)
        for i in range(len(substitution_positions)):
          if substitution_positions[i] not in ref_donor_diffs:
	    if substitution_positions[i] + 1 in insertion_positions_flat \
		or substitution_positions[i] - 1 in insertion_positions_flat \
		or substitution_positions[i] + 1 in deletion_positions_flat \
		or substitution_positions[i] - 1 in deletion_positions_flat :
		list_align_seq[substitution_positions[i]+row.read_offset] = row.align_seq[substitution_positions[i]+row.read_offset]
	    else: 
		list_align_seq[substitution_positions[i]+row.read_offset] = args.amplicon_seq[substitution_positions[i]]
        # now output correctect read
	s = ''.join(list_align_seq)
        fh_nhej_corrected.write("%s\n%s\n%s\n\n" % (row.ref_seq,row.align_str,s))
		 ...
		 #info("START=====================================================")
			...
			s = ''.join(list_align_seq)
        		fh_nhej_corrected.write("%s\n%s\n%s\n\n" % (row.ref_seq,row.align_str,s))
                    ...
                    #UNMODIFIED--EEK allows sequencing errors into "UNMODIFIED"...
                    else:
...
def main():
    try:
	     ...
	     global ref_donor_diffs
             global fh_ref,fh_sub,fh_hdr,fh_index,fh_mixed,fh_cutsub,fh_indel,fh_nhej_corrected
	     ...
	     # if cut_points > 1/2 len of amplicon, then offset = amplen - cutsite, else cutsite
             ...
             filename=os.path.join(os.path.abspath(args.output_folder),args.name,'nhej_corrected_reads.txt')
             fh_nhej_corrected = open(filename,"w")

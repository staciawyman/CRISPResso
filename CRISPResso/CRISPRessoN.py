...
nt_complement=dict({'A':'T','C':'G','G':'C','T':'A','N':'N','H':'H','_':'_','-':'-'})

def reverse_complement(seq):
        return "".join([nt_complement[c] for c in seq.upper()[-1::-1]])

def find_wrong_nt(sequence):
    return list(set(sequence.upper()).difference(set(['A','T','C','G','N','H'])))
...
def classify_read (row,substitution_positions,insertion_positions_flat,deletion_positions_flat):
    ...
    ref_donor_diffs=[i for i in xrange(len(args.expected_hdr_amplicon_seq)) if (args.expected_hdr_amplicon_seq[i] != 'N' and args.amplicon_seq[i] != 'N' and args.expected_hdr_amplicon_seq[i] != args.amplicon_seq[i])]
...
def plot_alleles_table(offset_to_plot,reference_seq,cut_point,df_alleles,sgRNA_name,OUTPUT_DIRECTORY,MIN_FREQUENCY=0.5,MAX_N_ROWS=100):
    ...
    dna_to_numbers={'-':0,'A':1,'T':2,'C':3,'G':4,'N':5,'H':6}
